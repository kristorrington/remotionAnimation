...
const WorkflowNode: React.FC<{ node: NodeSpec; x: number; delay: number; nw: number; nh: number; bandH: number }> = ({ node, x, delay, nw, nh, bandH }) => {
  ...
  // labels stay ONE line — long labels shrink instead of wrapping
  const fontSize = Math.min(34 * (nh / 104), (nw - 44) / (0.62 * Math.max(1, node.label.length)));
  return (
    <div
      style={{
        position: "absolute",
    ...
    >
      <span style={{ fontFamily: FONT, fontWeight: 800, fontSize, letterSpacing: 1, color: WHITE, textTransform: "uppercase", whiteSpace: "nowrap" }}>{node.label}</span>
    </div>
  );
};
...
export const WorkflowChain: React.FC<{ nodes: NodeSpec[]; retry?: [number, number]; retryColor?: string; retryLabel?: string; startAt?: number; width?: number; nodeAts?: number[]; scale?: number }> = ({ nodes, retry, retryColor = RED, retryLabel = "RETRY", startAt = 0, width = 1500, nodeAts, scale = 1 }) => {
  const frame = useCurrentFrame();
  const n = nodes.length;
  const nw = Math.round(NODE_W * scale);
  const nh = Math.round(NODE_H * scale);
  const bandH = Math.round(BAND_H * scale);
  const gap = n > 1 ? (width - n * nw) / (n - 1) : 0;
  const xOf = (i: number) => i * (nw + gap);
  const cy = bandH / 2;
  ...
  return (
    <div style={{ position: "relative", width, height: bandH + (retry ? 120 : 0), margin: "0 auto" }}>
      {/* connectors first (behind nodes) */}
      {nodes.slice(0, -1).map((_, i) => (
        <Connector key={`c-${i}`} x1={xOf(i) + nw} x2={xOf(i + 1)} y={cy} delay={nodeAts ? nodeAts[i + 1] - 12 : startAt + 10 + i * 12} danger={nodes[i + 1].state === "error"} />
      ))}
      {/* retry loop */}
      {retryPath && (
        <svg width={width} height={bandH + 120} style={{ position: "absolute", left: 0, top: 0, pointerEvents: "none", overflow: "visible" }}>
          ...
          {retryDraw > 0.5 && (
            <text x={(xOf(retry![0]) + xOf(retry![1])) / 2 + nw / 2} y={cy + nh / 2 + 88} fill={retryColor} fontFamily={FONT} fontSize="26" fontWeight="800" textAnchor="middle" opacity={retryDraw}>
              {retryLabel}
            </text>
          )}
        </svg>
      ...
      {/* nodes on top */}
      {nodes.map((node, i) => (
        <WorkflowNode key={`n-${i}`} node={node} x={xOf(i)} delay={nodeAts?.[i] ?? startAt + i * 12} nw={nw} nh={nh} bandH={bandH} />
      ))}
    </div>
  );
};
