...
export const R = {
  // Show the ENTIRE VS Code window (Kris): the window is a clean rectangle to
  // x≈1880 (the studio bookshelf/mic/lamp are all to the RIGHT of that), with a
  // thin Windows taskbar at the very bottom. Crop to the window edge (1876, a
  // 4px margin off the studio) × height 1055 (drops the bottom taskbar) — 16:9,
  ...
  APP_CARD: { x: 1276, y: 352, w: 468, h: 452 } as Rect, // just the finished Password Generator card
...
export type Layout = "screen" | "face" | "pip" | "split" | "appteaser";
export type Shot = {
  from: number;
  to: number;
  layout: Layout;
  ...
  screenFrozenImg?: string; // freeze the SCREEN on a still (skip a torn source segment)
};
...
// `frozenImg` shows a still frame instead of the video (used to skip a torn
// source segment on an idle screen).
const RegionView: React.FC<{ src: string; showFrame: number; rect: Rect; box: Rect; radius?: number; scaleBoost?: number; highlights?: HL[]; frozenImg?: string }>
  = ({ src, showFrame, rect, box, radius = 0, scaleBoost = 1, highlights, frozenImg }) => {
  ...
  return (
    <div style={{ position: "absolute", left: box.x, top: box.y, width: box.w, height: box.h, overflow: "hidden", borderRadius: radius }}>
      <div style={{ position: "absolute", width: vidW, height: vidH, left: offX, top: offY }}>
        {frozenImg
          ? <Img src={staticFile(frozenImg)} style={{ width: "100%", height: "100%" }} />
...
const ShotView: React.FC<{ src: string; shot: Shot }> = ({ src, shot }) => {
  ...
  if (shot.layout === "face") {
  ...
  } else if (shot.layout === "appteaser") {
    // face live, with the finished app card (from later footage) zooming in on the right
    const e = spring({ frame, fps, config: { stiffness: 130, damping: 15, mass: 0.9 }, durationInFrames: 28 });
    const box = { x: 1112, y: 232, w: 640, h: 618 };
    body = (
      <>
        <RegionView src={src} showFrame={show} rect={R.FACE} box={FULL} />
        <div style={{ position: "absolute", left: box.x, top: box.y, width: box.w, height: box.h, borderRadius: 22, overflow: "hidden", border: "3px solid rgba(255,255,255,0.92)", boxShadow: "0 30px 80px rgba(0,0,0,0.6)", transform: `scale(${interpolate(e, [0, 1], [0.72, 1])})`, transformOrigin: "70% 50%", opacity: interpolate(frame, [0, 10], [0, 1], CLAMP) }}>
          <RegionView src={src} showFrame={5100} rect={R.APP_CARD} box={{ x: 0, y: 0, w: box.w, h: box.h }} />
        </div>
      </>
    );
    ...
    body = (
      <>
        <RegionView src={src} showFrame={show} rect={shot.zoom ?? R.SCREEN} box={FULL} scaleBoost={push} highlights={shot.highlights} frozenImg={shot.screenFrozenImg} />
    ...
    body = <RegionView src={src} showFrame={show} rect={shot.zoom ?? R.SCREEN} box={FULL} scaleBoost={push} highlights={shot.highlights} frozenImg={shot.screenFrozenImg} />;
  }
