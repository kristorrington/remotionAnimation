...
export type Stat = { value: number; prefix?: string; suffix?: string; decimals?: number; label: string; at: number; accent?: string };
...
export const StatCountersScene: React.FC<{ durationInFrames: number; kicker?: string; title: string; stats: Stat[]; accent?: string; tint?: string }> = ({
  durationInFrames,
  kicker,
  title,
  stats,
  ...
  return (
    <SceneShell durationInFrames={durationInFrames} particleSeed={0x77c} tint={tint}>
        ...
        <div style={{ display: "flex", gap: 36, alignItems: "stretch" }}>
          {stats.map((s) => (
            <FloatingPanel key={s.label} delay={s.at} accent={s.accent ?? accent} style={{ width: 340, display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
              <AnimatedCounter to={s.value} at={s.at + 4} prefix={s.prefix} suffix={s.suffix} decimals={s.decimals} color={s.accent ?? accent} size={96} />
