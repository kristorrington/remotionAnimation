...
export const KineticClip: React.FC<{ durationInFrames: number; clip: string; clipDur?: number; text: string; highlight?: string; size?: number; source?: string; textAt?: number; startFrom?: number; bright?: number }>
  = ({ clip, clipDur, text, highlight, size = 96, source, textAt = 4, startFrom, bright = 0.5 }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const slam = spring({ frame: frame - textAt, fps, config: { stiffness: 190, damping: 22, mass: 0.9 }, durationInFrames: 28 });
  const push = interpolate(frame, [0, 200], [1.04, 1.1], CLAMP);
  const words = text.split(" ");
  const hnorm = (highlight ?? "").replace(/[^A-Za-z0-9]/g, "").toUpperCase();
  const vid = <OffthreadVideo src={staticFile(clip)} muted playbackRate={1.05} startFrom={startFrom} style={{ width: "100%", height: "100%", objectFit: "cover", filter: `brightness(${bright}) contrast(1.06) saturate(1.05)` }} />;
  return (
    <AbsoluteFill style={{ backgroundColor: "#0B0A09" }}>
      ...
      <AbsoluteFill style={{ alignItems: "center", justifyContent: "center" }}>
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", alignItems: "center", gap: Math.round(size * 0.26), maxWidth: 1600, transform: `scale(${interpolate(slam, [0, 1], [1.18, 1])})`, opacity: interpolate(frame, [textAt, textAt + 12], [0, 1], CLAMP) }}>
          {words.map((w, i) => {
