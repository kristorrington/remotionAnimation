...
import { ReposVideo, REPOS_WINDOWS, REPOS_FULLSCREEN, REPOS_EXTRA_CUTS } from "./ReposVideo";
...
// Final combined cut: talking head + repos receipt track + per-span PiP.
...
const COVERS = [...REPOS_WINDOWS].sort((a, b) => a.from - b.from);
...
const FULL = [...REPOS_FULLSCREEN].sort((a, b) => a.from - b.from);
...
const CUTS = [...new Set([...FULL.map((f) => f.from), ...REPOS_EXTRA_CUTS])].sort((a, b) => a - b);

const CLAMP = { extrapolateLeft: "clamp", extrapolateRight: "clamp" } as const;

export const ReposFinal: React.FC = () => {
  ...
  return (
    ...
    <AbsoluteFill style={{ backgroundColor: "black" }}>
      ...
      {/* VO boost 2.8× (source peaks ≈ −12.4 dB — probed 2026-07-17) */}
      <AbsoluteFill
        style={{
      ...
      >
        {/* jump-cuts retimed to THIS footage's sentence gaps (2026-07-17) */}
        <FootageDirector
          footage={FOOTAGE}
          volume={2.8}
          framing={[
            { at: 0, scale: 1.05, y: -1 },
            { at: 717, scale: 1.1, y: -2 },
            { at: 1491, scale: 1.0, y: 0 },
            { at: 2237, scale: 1.08, y: -1 },
            { at: 3229, scale: 1.12, y: -2 },
            { at: 3693, scale: 1.0, y: 0 },
            { at: 4485, scale: 1.07, y: -1 },
            { at: 5373, scale: 1.12, y: -2 },
            { at: 6351, scale: 1.05, y: -1 },
            { at: 7150, scale: 1.0, y: 0 },
          ]}
        />
      </AbsoluteFill>
